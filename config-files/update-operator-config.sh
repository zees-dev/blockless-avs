...
OUTPUT_DEPLOYMENT_FILE=../contracts/script/output/$CHAINID/blockless_avs_deployment_output.json
...
AVS_SERVICE_MANAGER=$(jq -r '.addresses.blocklessAVSServiceManager' $OUTPUT_DEPLOYMENT_FILE)
...
sed -i '' "s|avs_service_manager_addr: .*|avs_service_manager_addr: $AVS_SERVICE_MANAGER # blocklessAVSServiceManager|" $OPERATOR_CONFIG_FILE
